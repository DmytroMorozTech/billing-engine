...
| `biome.jsonc` | Biome formats `next-env.d.ts`, a Next.js-generated file marked do-not-edit, and lints the Jest coverage report | add `files.includes` with `!next-env.d.ts`, `!.next`, `!__coverage__`, `!__reports__`. Written here originally with `/**` suffixes; Biome's own formatter rewrites them to the bare form on every `biome check --write`, and both check exactly the same 32 files, so the bare form is the canonical one for this version and is what the file now holds — and **no** catch-all entry, since Foundry's shared config already provides one (`lint/suspicious/noBiomeFirstException`) |

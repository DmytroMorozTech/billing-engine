...
const env = { DATABASE_URL: 'postgres://billing@postgres:5432/billing', REDIS_URL: 'redis://redis:6379' };
...
describe('loadConfig', () => {
  ...
  it('refuses to start without a database, and says which variable', () => {
    expect(() => loadConfig({ REDIS_URL: env.REDIS_URL })).toThrow('DATABASE_URL');
  });
  ...
  it('refuses to start without a transport to publish to', () => {
    ...
    expect(() => loadConfig({ DATABASE_URL: env.DATABASE_URL })).toThrow('REDIS_URL');
  });
